...
abstract class TraversalTest<T extends Traversal<Integer>> {
    ...
    private void testContents(T graph, List<Integer> traversal) {
    ...
    private void testCyclicGraph(T graph,List<Integer> traversal) {
    ...
    private void testPairedGraph(T graph,List<Integer> traversal) {
        if (graph.getNoOfNodes() == 0) return;
    ...
    @Test
    void testEmpty() {
        testContents(newTraversal(),new ArrayList<>());
    }
