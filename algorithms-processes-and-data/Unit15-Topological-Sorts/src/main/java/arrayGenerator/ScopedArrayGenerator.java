...
public abstract class ScopedArrayGenerator<T> implements ArrayGenerator<T> {
    ...
    /**
     * Generate a random array in which each entry appears only once.
     *
     * @param size the size of the array to be generated.
     * @return an array of the required size, containing random values from within the
     * permitted scope.  No value may appear more than once.
     *
     * @throws NegativeArraySizeException if the size given is negative.
     */
    public T[] getNoDuplicatesArray(int size) throws NegativeArraySizeException, TooSmall {
        if (size < 0) {
            throw new NegativeArraySizeException();
        }
        if (size > scope.size()) {
            throw new TooSmall("Not enough different elements in the scope to create non-duplicate array");
        }
        T[] array = createArray(size);
        boolean inArray;
        T value;
